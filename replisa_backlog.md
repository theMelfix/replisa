...
| 4.2.6 | Sezione Billing: piano attivo, conteggio messaggi, upgrade | `P2` | 3 | 🟡 | Stripe via **Laravel Cashier** (Billable su `Tenant`). Pagina `/billing`: 4 piani → Stripe Checkout ospitato + Billing Portal per gestione/disdetta; webhook auto-registrato. `config/plans.php` (Price ID da `.env`). Riquadro "Utilizzo" con conteggio messaggi inviati/ricevuti del mese + contatti vs limite. Enforcement limiti via `App\Support\PlanLimits` (automazioni attive + contatti nuovi su form/CSV; webhook inbound escluso). **Gating per piano** (`allows()`: campagne/scadenze da Base) e **add-on Recensioni** (`hasReviewsAddon()`: Business o concesso). **Setup Stripe fatto in TEST** (account + 4 prodotti + add-on + 9 prezzi; Price ID in `.env`/overlay). **Fix checkout Livewire** (`Checkout::redirect()`/`redirectToBillingPortal()` davano TypeError in Livewire → uso di `$this->redirect($url)`). **Billing verificato e2e su prod** (test mode): checkout + webhook (`pending_webhooks=0`) + Billing Portal ok su replisa.com. Manca: passaggio a **LIVE** (attivazione account/P.IVA, IVA `exclusive`→prezzi da ricreare, Price ID/chiavi/webhook live) — vedi `stripe-billing-status`. 2026-06-20/22, 2026-07-28 |
